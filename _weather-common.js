...
function cToF(value) {
  return value == null ? null : Math.round((Number(value) * 9 / 5 + 32) * 10) / 10;
}
function msToMph(value) {
  return value == null ? null : Math.round(Number(value) * 2.236936 * 10) / 10;
}
...
function hourlyArray(data) {
  return Array.isArray(data?.timelines?.hourly) ? data.timelines.hourly : [];
}
...
async function getDelawareAlerts() {
  const response = await fetch("https://api.weather.gov/alerts/active?area=DE", {
    headers: { "User-Agent": "DelawareWeatherGuy/1.0 contact@delawareweatherguy.example", "Accept": "application/geo+json" }
  });
...
async function buildSnapshot() {
  ...
  const alerts = await getDelawareAlerts();
  return { generatedAt: new Date().toISOString(), locations, alerts };
}
module.exports = { LOCATIONS, buildSnapshot };
